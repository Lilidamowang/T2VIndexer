...
class T5FineTuner(pl.LightningModule):
    ...
    def validation_step_i(self, batch, i):
        ...
        if self.args.decode_embedding == 1:
            ...
            si = batch["source_ids"].cuda()
            sm = batch["source_mask"].cuda()
            start_time = time.time()
            self.args.num_return_sequences = 40
            outs, scores = self.model.generate(
                si,
                attention_mask=sm,
                use_cache=False,
            ...
            self.all_execution_time_ms = self.all_execution_time_ms + execution_time_ms
    ...
    def validation_epoch_end(self, outputs):  
        ...
        for item in res1:
            ...
            for idx in range(LAYERS):
                if gt in pre[0:idx+1]: # 一阶段正确
                    count[idx] += 1
        with open('my_file.txt', 'a') as file:
            file.write(f"===============================================================\n")
            for i in range(LAYERS):
                print(f"{i+1}:{count[i]}", end=", ")
                file.write(f"{i+1}:{count[i]}, ")
            file.write("\n")
            
        self.log("recall1", count[10])
    ...
    def train_dataloader(self): # 调用了
        ...
        if hasattr(self, 'l1_query_train_dataset'):
            ...
            train_dataset = MainDataset(self.args, self.tokenizer)
        # print("the_dataset_len:", len(train_dataset))
