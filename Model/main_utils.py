...
def load_data_diy(args, path):
    ...
    train_file = path
    hues.info(f"load training data from {train_file}")
    df = pd.read_csv(
        train_file,
        encoding='utf-8',
        header=None, sep='\t')
...
def load_data_infer_diy(args,path):
    test_file = path
    hues.info(f"load testing data from {test_file}")
    df = pd.read_csv(
                    test_file,
                    encoding='utf-8',
                    header=None, sep='\t')
